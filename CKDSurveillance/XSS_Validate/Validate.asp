...
<%
'03/24/08 (SD): Copied the latest code from ValidateMaster; Turned ON SQL Injection by default.
...
If trim(XSS_Validated) = "" then
	...
	'06/27/08: 'Turned SQL Injection to ON based on meeting with Amy and Priya.
	...
	Function FoundSuspectTags(Byval InputString, Byval chkCSS, Byval chkSQLInj, ByVal chkQueryStr)
		...
		'06/27/08: Only single quotes, xp_, sp_, and double hyphens were decided to be implemented based on meeting with Amy and Priya.
		...
		For each frmItem in Request.Form
			For iCount = 1 to Request.Form(frmItem).Count
				...
				If FoundSuspectTags(iNameValue, chkCSS, chkSQLInj, False) then
					'Response.Write "Forms Collection" & "<BR>"
					'Response.Write "iNameValue=" & iNameValue
					'Response.End
					Set objRE = Nothing
		...
		For each qryItem in Request.QueryString
			For iCount = 1 to Request.QueryString(qryItem).Count
				...
				If FoundSuspectTags(iNameValue, chkCSS, chkSQLInj, True) then
					'Response.Write "Query Collection" & "<BR>"
					'Response.Write "iNameValue=" & iNameValue
					'Response.End
					Set objRE = Nothing
					If trim(BlockedPage) <> "" then
